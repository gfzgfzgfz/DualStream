...
class CoarseGrainedDataset(Dataset):
    ...
    def _load_data(self):
        ...
        # 统计切片级标签分布
        slice_label_counts = {0: 0, 1: 0}
        for slice_labels in self.func_slice_labels:
            for label in slice_labels:
                slice_label_counts[label] += 1
                
        logging.info(f"加载完成: {len(self.func_codes)}个函数")
        logging.info(f"函数级 - 正样本: {len(positive_indices)}, 负样本: {len(selected_negative)}")
        logging.info(f"切片级 - 正样本: {slice_label_counts[1]}, 负样本: {slice_label_counts[0]}")
        ...
        logging.info(f"切片标签分布: 非漏洞={slice_label_counts[0]}, 漏洞={slice_label_counts[1]}")
    ...
    def __getitem__(self, idx):
        ...
        # 对函数代码进行编码
        func_encoded = self.func_tokenizer(
            func_code,
        ...
        # 对所有切片一次性进行编码
        slice_encoded = self.slice_tokenizer(
            slice_codes,
            add_special_tokens=True,
            max_length=self.max_length,
            padding='max_length',
        ...
        # 确保 slice_labels 长度正确（无论是否填充）
        slice_labels = slice_labels[:self.max_slices_per_func]  # 截断到最大长度
        # 如果还不够，继续填充（处理边界情况）
        while len(slice_labels) < self.max_slices_per_func:
...
def create_coarse_dataloaders(data_root, func_tokenizer, slice_tokenizer, batch_size=8, max_length=512, num_workers=4, pos_neg_ratio=2.0, max_slices_per_func=10, use_cache=True, pin_memory=True, persistent_workers=True):
    """创建粗粒度数据加载器"""
    # 创建数据集
